...
while True:
    ...
    if activate_logic == True:
        if check == 1:
            ...
            for (x, y, w, h) in faces:
                ...
                # identify open eyes
                open_eyes_detected = 0
                eyes_tilt = eye_cascade.detectMultiScale(
                    gray[y : y + h, x : x + w], 1.1, 4
                )
                eyes = eye_cascade.detectMultiScale(roi_gray, 1.25, 5)
                index = 0
                ...
                for (ex, ey, ew, eh) in eyes_tilt:
                    if index == 0:
                        ...
                        eye_2 = [ex, ey, ew, eh]
                    # cv2.rectangle(img[y:y + h, x:x + w], (ex, ey), (ex
                    #             + ew, ey + eh), (0, 0, 255), 2)
                    index = index + 1
                ...
                smile, rejectLevels, levelWeights = smile_cascade.detectMultiScale3(
                    roi_gray, 1.25, 5, outputRejectLevels=True
                )
    ...
    # os.killpg(p.pid, signal.SIGTERM)
    # cv2.destroyAllWindows()
    # cv2.imwrite('Frame2444.jpg',img)
    # with Image.open('Frame2444.jpg') as img:
    # img.load()
    # cropped_img = img.crop((x-40, y-40, x + w + 40, y + h + 40))
    # cropped_img.show()
    # subprocess.run(['libcamera-still --width 640 --height 480 -o 64mp-autofocus-test-new.jpg --autofocus'], shell=True)
    #                         sys.exit(1)
